...
public class PromptManager {
    ...
    public String buildSystemPrompt(JsonObject personality, String languageCode) {
        ...
        com.adenium.zanatenunchi.lang.IBotLanguageProvider provider = LanguageManager.getProvider(languageCode);

        String gender = safeGetString(personality, "gender", "unknown");
        String botName = safeGetString(personality, "name", "Bot");
        String traits = safeGetFlexibleString(personality, "traits", "analytical");
        String style = safeGetFlexibleString(personality, "speakingStyle", "direct");
        String age = safeGetString(personality, "age", "25");

        String emotionalBias = buildPrimaryTraitBias(personality, traits);

        // Delegate the main prompt building to the specific language provider
        String systemPrompt = provider.buildSystemPrompt(botName, gender, age, traits, style, emotionalBias);
        
        // Append overarching language/dialect rules
        return systemPrompt + "\n\nDIALECT/LANGUAGE CONTEXT: " + lang.getDialectInstructions() +
               "\n\nRespond ONLY with the chat message. Target language: " + lang.languageName();
    }
    ...
    public String buildShortPrompt(JsonObject personality, String languageCode) {
        return buildSystemPrompt(personality, languageCode) +
                "\n\nINSTRUCTION: " + LanguageManager.getProvider(languageCode).getShortInstruction() +
                " Use ONLY data from the event; DO NOT invent, hallucinate or explain anything.";
    }
    ...
    public String buildNormalPrompt(JsonObject personality, String languageCode) {
        return buildSystemPrompt(personality, languageCode) +
                "\n\nINSTRUCTION: " + LanguageManager.getProvider(languageCode).getNormalInstruction() +
                " Keep it to 1 or 2 sentences max. You can ask a short question if it fits. " +
                "Address the player directly using ONLY facts from the event; do not add external context.";
    }
    ...
    public String buildEmotivePrompt(JsonObject personality, String languageCode) {
        return buildSystemPrompt(personality, languageCode) +
                "\n\nINSTRUCTION: " + LanguageManager.getProvider(languageCode).getEmotiveInstruction();
    }
